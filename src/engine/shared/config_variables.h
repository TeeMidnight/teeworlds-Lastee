...
MACRO_CONFIG_STR(SvMapHttpDownloadDir, sv_map_http_download_dir, 256, "", CFGFLAG_SAVE|CFGFLAG_SERVER, "Link dir for DDNet Http download (need add '/' by youself)")
...
MACRO_CONFIG_INT(SvMaxClients, sv_max_clients, 16, 1, MAX_PLAYERS, CFGFLAG_SAVE|CFGFLAG_SERVER, "Maximum number of clients that are allowed on a server")
MACRO_CONFIG_INT(SvMaxClientsPerIP, sv_max_clients_per_ip, 2, 1, MAX_PLAYERS, CFGFLAG_SAVE|CFGFLAG_SERVER, "Maximum number of clients with the same IP that can connect to the server")
